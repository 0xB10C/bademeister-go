...
func NewBitcoinRPCClient(rpcUser, rpcPass, rpcHost, rpcPort string) (*BitcoinRPCClient, error) {
	if len(rpcUser) == 0 || len(rpcPass) == 0 || len(rpcHost) == 0 || len(rpcPort) == 0 {
		return nil, fmt.Errorf(
			"received empty parameter (rpcUser=%q rpcPass=%q rpcHost=%q rpcPort=%q)",
			rpcUser, rpcPass, rpcHost, rpcPort,
		)
	}
